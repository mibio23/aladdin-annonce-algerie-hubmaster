...
// Helper function to extract categories from TypeScript files
function loadCategoryFile(filePath) {
  try {
    const content = fs.readFileSync(filePath, 'utf8');
    
    // Find the export line
    const exportLineMatch = content.match(/export const main\d+Categories\w*\s*:\s*MainCategory\[\s]*=\s*/);
    if (!exportLineMatch) {
      console.error(`Could not find export line in ${filePath}`);
      return [];
    }
    
    // Find the start and end of the array
    const startIndex = exportLineMatch.index + exportLineMatch[0].length;
    let braceCount = 0;
