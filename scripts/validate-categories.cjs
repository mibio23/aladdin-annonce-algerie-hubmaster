...
// Helper function to read and parse TypeScript category files
function loadCategoryFile(filePath) {
  try {
    const content = fs.readFileSync(filePath, 'utf8');
    
    // Extract the export array using regex
    const exportMatch = content.match(/export const main\d+Categories\w+: \[[\s\S]*?\];/);
    if (!exportMatch) {
      throw new Error(`Could not find export array in ${filePath}`);
    }
    
    // Remove TypeScript syntax and convert to valid JavaScript
    let jsCode = exportMatch[0]
      .replace(/export const main\d+Categories\w+: /, 'const categories = ')
      .replace(/export interface MainCategory \{[\s\S]*?\}/, '') // Remove interface
      .replace(/export \{ MainCategory \};/, '') // Remove export
      .replace(/MainCategory\[\]/g, '[]') // Replace type annotation
      .replace(/: MainCategory\[\]/g, '') // Remove type annotation
      .replace(/: string/g, '') // Remove string type annotations
      .replace(/: MainCategory/g, '') // Remove MainCategory type annotations
      .replace(/\?/g, ''); // Remove optional operators
    
    // Execute the JavaScript code in a safe context
    eval(jsCode);
    return categories;
  } catch (error) {
...
const main27CategoriesAr = loadCategoryFile(path.join(__dirname, '../src/data/categories/main27CategoriesAr.ts'));
const main27CategoriesEn = loadCategoryFile(path.join(__dirname, '../src/data/categories/main27CategoriesEn.ts'));
const main27CategoriesDe = loadCategoryFile(path.join(__dirname, '../src/data/categories/main27CategoriesDe.ts'));
const main27CategoriesEs = loadCategoryFile(path.join(__dirname, '../src/data/categories/main27CategoriesEs.ts'));
