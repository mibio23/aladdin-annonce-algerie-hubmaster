...
async function loadCategoryFile(filePath) {
  try {
    const content = fs.readFileSync(filePath, 'utf8');
    
    // Create a temporary JavaScript file by removing TypeScript syntax
    const tempJsPath = filePath.replace('.ts', '.temp.js');
    
    // Remove TypeScript-specific syntax
    let jsContent = content
      // Remove interface definitions
      .replace(/export interface MainCategory\s*\{[\s\S]*?\}/g, '')
      // Replace export const with const
      .replace(/export const main\d+Categories\w*\s*:/g, 'const mainCategories:')
      // Remove type annotations
      .replace(/: MainCategory\[\]/g, '')
      .replace(/: string/g, '')
      .replace(/: MainCategory/g, '')
      .replace(/\?/g, '')
      // Add module.exports at the end
      .replace(/const mainCategories/g, 'module.exports.mainCategories');
    
    // Write temporary JavaScript file
    fs.writeFileSync(tempJsPath, jsContent);
    
    // Use dynamic import to load the module
    const moduleRequire = createRequire(tempJsPath);
    const tempModule = moduleRequire(tempJsPath);
    
    // Clean up temporary file
    fs.unlinkSync(tempJsPath);
    
    return tempModule.mainCategories || [];
  } catch (error) {
...
async function main() {
  ...
  try {
    ...
    const main27CategoriesAr = await loadCategoryFile(path.join(__dirname, '../src/data/categories/main27CategoriesAr.ts'));
    const main27CategoriesEn = await loadCategoryFile(path.join(__dirname, '../src/data/categories/main27CategoriesEn.ts'));
    const main27CategoriesDe = await loadCategoryFile(path.join(__dirname, '../src/data/categories/main27CategoriesDe.ts'));
    const main27CategoriesEs = await loadCategoryFile(path.join(__dirname, '../src/data/categories/main27CategoriesEs.ts'));
