...
// Helper function to extract categories from TypeScript files using regex
function loadCategoryFile(filePath) {
  try {
    const content = fs.readFileSync(filePath, 'utf8');
    
    // Extract the array content using regex
    const arrayMatch = content.match(/export const main\d+Categories\w*\s*:\s*MainCategory\[\s]*=\s*(\[[\s\S]*?\]);/s);
    if (!arrayMatch) {
