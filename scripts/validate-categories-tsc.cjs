...
// Helper function to compile and load TypeScript category files
function loadCategoryFile(filePath) {
  try {
    // Create a temporary compiled JS file
    const tempJsPath = filePath.replace('.ts', '.temp.js');
    
    // Compile TypeScript to JavaScript
    execSync(`npx tsc ${filePath} --outFile ${tempJsPath} --target es2020 --module commonjs`, { stdio: 'inherit' });
    ...
    return tempModule.main27Categories || [];
  } catch (error) {
