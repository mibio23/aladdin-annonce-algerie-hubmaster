...
function loadCategoryFile(filePath) {
  try {
    ...
    // Create a module-like environment to evaluate the TypeScript code
    const moduleExports = {};
    
    // Remove TypeScript-specific syntax and create a JavaScript version
    let jsContent = content
      // Remove interface definitions
      .replace(/export interface MainCategory\s*\{[\s\S]*?\}/g, '')
      // Remove export keyword from const declarations
      ...
      .replace(/export const mainCategories/g, 'module.exports.mainCategories');
    
    // Evaluate the modified content
    eval(jsContent);
    
    return moduleExports.mainCategories || [];
  } catch (error) {
