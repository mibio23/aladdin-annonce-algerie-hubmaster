...
console.log('🔗 INTÉGRATION DE "Matériel Professionnel Médicale"');
console.log('================================================================================');
...
const newCategoryPath = path.join(process.cwd(), 'materiel-medical-category.ts');
const newCategoryContent = fs.readFileSync(newCategoryPath, 'utf-8');

console.log('✅ Fichiers lus avec succès');

// Recherche de la fin complète de la section "Santé & Beauté"
const santeBeautePattern = /id:\s*'sante-beaute'[\s\S]*?subcategories:\s*\[[\s\S]*?\]\s*}\s*},?/g;
const santeBeauteMatch = fileContent.match(santeBeautePattern);

if (!santeBeauteMatch) {
  console.error('❌ Erreur: Impossible de trouver la section complète "Santé & Beauté"');
  
  // Alternative: chercher la fin de la section par une autre méthode
  const debutSection = fileContent.indexOf("id: 'sante-beaute'");
  if (debutSection === -1) {
    console.error('❌ Erreur: Section "Santé & Beauté" non trouvée');
    process.exit(1);
  }
  ...
  if (finSection > debutSection) {
    console.log('✅ Section "Santé & Beauté" trouvée par méthode alternative');
    ...
    console.log('✅ Catégorie "Matériel Professionnel Médicale" intégrée avec succès');
  ...
  console.log('✅ Section "Santé & Beauté" trouvée par regex');
  
  // Extraction de la position d'insertion
  const santeBeauteIndex = fileContent.indexOf(santeBeauteMatch[0]);
  if (santeBeauteIndex === -1) {
    console.error('❌ Erreur: Impossible de localiser la position d\'insertion');
    process.exit(1);
  }
  
  const insertionPosition = santeBeauteIndex + santeBeauteMatch[0].length;
...
const categorieIntegree = verificationContent.includes('id: \'materiel-professionnel-medical\'');
...
const nouvelleCategoriePattern = /id:\s*'materiel-professionnel-medical'[\s\S]*?subcategories:\s*\[[\s\S]*?\]\s*}\s*}/g;
