...
console.log('🔗 INTÉGRATION DE "Matériel Professionnel de Surveillance"');
console.log('================================================================================');
...
const newCategoryPath = path.join(process.cwd(), 'materiel-surveillance-category.ts');
const newCategoryContent = fs.readFileSync(newCategoryPath, 'utf-8');

console.log('✅ Fichiers lus avec succès');

// Recherche de la fin complète de la section "Matériel Professionnel d'Incendie"
const materielIncendiePattern = /id:\s*'materiel-professionnel-incendie'[\s\S]*?subcategories:\s*\[[\s\S]*?\]\s*}\s*},?/g;
const materielIncendieMatch = fileContent.match(materielIncendiePattern);

if (!materielIncendieMatch) {
  console.error('❌ Erreur: Impossible de trouver la section complète "Matériel Professionnel d\'Incendie"');
  
  // Alternative: chercher la fin de la section par une autre méthode
  const debutSection = fileContent.indexOf("id: 'materiel-professionnel-incendie'");
  if (debutSection === -1) {
    console.error('❌ Erreur: Section "Matériel Professionnel d\'Incendie" non trouvée');
    process.exit(1);
  }
  ...
  if (finSection > debutSection) {
    console.log('✅ Section "Matériel Professionnel d\'Incendie" trouvée par méthode alternative');
    ...
    console.log('✅ Catégorie "Matériel Professionnel de Surveillance" intégrée avec succès');
  ...
  console.log('✅ Section "Matériel Professionnel d\'Incendie" trouvée par regex');
  
  // Extraction de la position d'insertion
  const materielIncendieIndex = fileContent.indexOf(materielIncendieMatch[0]);
  if (materielIncendieIndex === -1) {
    console.error('❌ Erreur: Impossible de localiser la position d\'insertion');
    process.exit(1);
  }
  
  const insertionPosition = materielIncendieIndex + materielIncendieMatch[0].length;
...
const categorieIntegree = verificationContent.includes('id: \'materiel-professionnel-surveillance\'');
...
const nouvelleCategoriePattern = /id:\s*'materiel-professionnel-surveillance'[\s\S]*?subcategories:\s*\[[\s\S]*?\]\s*}\s*}/g;
