...
console.log('🔗 INTÉGRATION DE "Matériel Professionnel BTP"');
console.log('================================================================================');
...
const newCategoryPath = path.join(process.cwd(), 'materiel-btp-category.ts');
const newCategoryContent = fs.readFileSync(newCategoryPath, 'utf-8');

console.log('✅ Fichiers lus avec succès');

// Recherche de la fin complète de la section "Services Professionnels"
const servicesProfessionnelsPattern = /id:\s*'services-professionnels'[\s\S]*?subcategories:\s*\[[\s\S]*?\]\s*}\s*},?/g;
const servicesProfessionnelsMatch = fileContent.match(servicesProfessionnelsPattern);

if (!servicesProfessionnelsMatch) {
  console.error('❌ Erreur: Impossible de trouver la section complète "Services Professionnels"');
  
  // Alternative: chercher la fin de la section par une autre méthode
  const debutSection = fileContent.indexOf("id: 'services-professionnels'");
  if (debutSection === -1) {
    console.error('❌ Erreur: Section "Services Professionnels" non trouvée');
    process.exit(1);
  }
  ...
  if (finSection > debutSection) {
    console.log('✅ Section "Services Professionnels" trouvée par méthode alternative');
    ...
    console.log('✅ Catégorie "Matériel Professionnel BTP" intégrée avec succès');
  ...
  console.log('✅ Section "Services Professionnels" trouvée par regex');
  
  // Extraction de la position d'insertion
  const servicesProfessionnelsIndex = fileContent.indexOf(servicesProfessionnelsMatch[0]);
  if (servicesProfessionnelsIndex === -1) {
    console.error('❌ Erreur: Impossible de localiser la position d\'insertion');
    process.exit(1);
  }
  
  const insertionPosition = servicesProfessionnelsIndex + servicesProfessionnelsMatch[0].length;
...
const categorieIntegree = verificationContent.includes('id: \'materiel-professionnel-btp\'');
...
const nouvelleCategoriePattern = /id:\s*'materiel-professionnel-btp'[\s\S]*?subcategories:\s*\[[\s\S]*?\]\s*}\s*}/g;
