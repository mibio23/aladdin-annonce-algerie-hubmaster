...
console.log('🔍 Test final des catégories spécifiques à l\'Algérie...');
...
const sampleCategory = 'artisanat-traditionnel-algerien';
...
const testPassed = allCategoriesFound && structureValid;
...
const testResultsPath = path.join(__dirname, 'final-test-algerian-categories-results.json');
try {
  fs.writeFileSync(testResultsPath, JSON.stringify(testResults, null, 2));
  console.log(`\n📄 Résultats du test final sauvegardés dans: ${testResultsPath}`);
} catch (error) {
  console.error('❌ Erreur lors de l\'écriture des résultats du test final:', error.message);
}
